...
impl Texture {
  pub fn new_2d(
    ...
    unsafe {
      if let Some(app) = crate::core::app::APP_INSTANCE.as_mut() {
        app.render_device.create_texture(&wgpu::TextureDescriptor {
          label: None,
          size: wgpu::Extent3d {
            width: size.0,
            height: size.1,
  ...
  pub fn update(&self, data: &[u8]) {
    unsafe {
      if let Some(app) = crate::core::app::APP_INSTANCE.as_mut() {
        app.render_device.update_texture(self, data);
      }
    }
  }
}
...
impl Sampler {
  pub fn new(desc: &wgpu::SamplerDescriptor) -> Self {
    unsafe {
      if let Some(app) = crate::core::app::APP_INSTANCE.as_mut() {
        app.render_device.create_sampler(desc)
      } else {
        Self { handle: None }
      }
    }
